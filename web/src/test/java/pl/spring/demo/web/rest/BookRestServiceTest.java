...
public class BookRestServiceTest {
	...
	public void testShouldDeleteBook() throws Exception {
		...
		final String bookTitle = "testTitle";
		final Long id = 1L;
		final BookTo bookTo1 = new BookTo(id, bookTitle, "Author1");

		Mockito.when(bookService.deleteBookById(id)).thenReturn(bookTo1);
		// when
		ResultActions response = this.mockMvc.perform(
				delete("/book?id=" + id).accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON));
		// then
		Mockito.verify(bookService).deleteBookById(id);
		response.andExpect(status().isOk())
